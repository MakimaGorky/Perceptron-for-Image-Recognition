...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        private void InitCamera()
        {
            ...
            if (videoDevices.Count > 0)
            {
                ...
                {
                    // Добавляем камеры в ваш ComboBox (предположим его имя cmbVideoSource)
        ...
        // Обработчик кнопки "Старт камеры"
        private void btnStartCam_Click(object sender, EventArgs e)
        {
            // int index = cmbVideoSource.SelectedIndex;
            int index = 0; // Если комбобокса нет, берем первую
            if (videoDevices.Count > 0)
            {
        ...
        private void video_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            ...
            // Лучше добавить на форму pictureBoxCamera.
        }

        // Обработчик закрытия формы (важно остановить камеру)
        ...
        // === ГЛАВНОЕ: Обработчик кнопки "Распознать с камеры" ===
        ...
        private Bitmap lastFrame; // Поле класса
        // Обновите video_NewFrame:
        /*
        private void video_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            lastFrame = (Bitmap)eventArgs.Frame.Clone();
            pictureBoxCamera.Image = lastFrame; // pictureBoxCamera - добавьте на форму
        }
        */
    }
}